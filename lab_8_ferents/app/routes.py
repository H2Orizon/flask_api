...
@main.get("/{book_id}")
async def get_books(request: Request, book_id: str, current_user: User = Depends(get_current_user)):
    await rate_limit(request, str(current_user.username))
    try:
...
async def delet_book(request: Request, book_id: str, current_user: User = Depends(get_current_user)):
    await rate_limit(request, str(current_user.username))
    try:
        obj_id = ObjectId(book_id)
    except InvalidId:
